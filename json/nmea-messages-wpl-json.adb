with NMEA.Messages.JSON; pragma Warnings (Off, NMEA.Messages.JSON);
with NMEA.JSON; pragma Warnings (Off, NMEA.JSON);

package body NMEA.Messages.WPL.JSON is
   use NMEA.Messages.JSON;
   use NMEA.JSON;
   

   ----------------------------------------------------
   --  WPL_Message
   ----------------------------------------------------
   procedure Populate (Val : WPL_Message; Target : in out JSON_Value) is
   begin
      Set_Field (Target, "Latitude", Create (Val.Latitude));
      Set_Field (Target, "N_Or_S_North_Or_South", Create (Val.N_Or_S_North_Or_South));
      Set_Field (Target, "Longitude", Create (Val.Longitude));
      Set_Field (Target, "E_Or_W_East_Or_West", Create (Val.E_Or_W_East_Or_West));
      Set_Field (Target, "Waypoint_Name", Create (Val.Waypoint_Name));
   end;

   ------------
   -- Create --
   ------------

   function Create (Val : WPL_Message) return JSON_Value is
   begin
      return Ret : JSON_Value := Create_Object do
         Populate (Val, Ret);
      end return;
   end Create;

   ---------
   -- Get --
   ---------

   function Get (Val : GNATCOLL.JSON.JSON_Value) return WPL_Message is
   begin
      return Ret : WPL_Message do
         Map (Val, Ret);
      end return;
   end Get;

   ---------
   -- Get --
   ---------

   function Get
     (Val   : JSON_Value;
      Field : UTF8_String)
      return WPL_Message
   is
   begin
      return Get (JSON_Value'(Get (Val, Field)));
   end Get;

   ---------------
   -- Set_Field --
   ---------------

   procedure Set_Field
     (Val        : JSON_Value;
      Field_Name : UTF8_String;
      Field      : WPL_Message)
   is      
   begin
      Set_Field (Val, Field_Name, Create (Field));
   end Set_Field;

   ---------------
   -- Map_Field --
   ---------------
   procedure Map_Field (Name : UTF8_String; Value : JSON_Value; To : in out WPL_Message) is
   begin
      if Name = "Latitude" then
         To.Latitude := Get (Value);
      elsif Name = "N_Or_S_North_Or_South" then
         To.N_Or_S_North_Or_South := Get (Value);
      elsif Name = "Longitude" then
         To.Longitude := Get (Value);
      elsif Name = "E_Or_W_East_Or_West" then
         To.E_Or_W_East_Or_West := Get (Value);
      elsif Name = "Waypoint_Name" then
         To.Waypoint_Name := Get (Value);
      else  
         null;
      end if;
   end;
   
   ---------
   -- Map --
   ---------
   procedure Map
     (Val : JSON_Value;
      To  : in out WPL_Message)
   is
      procedure Process (Name : UTF8_String; Value : JSON_Value);
      procedure Process (Name : UTF8_String; Value : JSON_Value) is
      begin
         Map_Field (Name, Value, To);
      end;
   begin
      Map_JSON_Object (Val, Process'Access);
   end Map;

end NMEA.Messages.WPL.JSON;