...
package NMEA.Messages.Nmea_Hight_Test_Data is

--  begin read only
   type Test_Nmea_Hight is new
     GNATtest_Generated.GNATtest_Standard.NMEA.Messages.NMEA_Long_Float_Test_Data.NMEA_Long_Float_Tests.Test_NMEA_Long_Float
--  end read only
   with null record;

   procedure Set_Up (Gnattest_T : in out Test_Nmea_Hight);
   procedure Tear_Down (Gnattest_T : in out Test_Nmea_Hight);
