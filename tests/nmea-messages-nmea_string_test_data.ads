...
package NMEA.Messages.NMEA_String_Test_Data is

--  begin read only
   type Test_NMEA_String is new
     GNATtest_Generated.GNATtest_Standard.NMEA.Messages.NMEA_Field_Test_Data.NMEA_Field_Tests.Test_NMEA_Field
--  end read only
   with null record;

   procedure Set_Up (Gnattest_T : in out Test_NMEA_String);
   procedure Tear_Down (Gnattest_T : in out Test_NMEA_String);
